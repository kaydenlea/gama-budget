...
Sensitive functions should keep the shared auth guard and use the shared Supabase-backed rate-limit helper. The helper now enforces real backend state and still fails closed with the explicit blocker error if the backend is missing or broken. Do not bypass that shared path with ad hoc per-function logic.
