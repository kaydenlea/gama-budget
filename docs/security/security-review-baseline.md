...
- Supabase Edge Functions use shared helpers for method guards, code-owned JWT verification on authenticated scaffolds, a shared Supabase-backed rate-limit backend for sensitive functions that fails closed if unavailable, and user-safe error responses
...
- sensitive or abuse-prone Edge Function work:
  - classify whether the shared rate-limit backend and per-function policy are sufficient or need strengthening
